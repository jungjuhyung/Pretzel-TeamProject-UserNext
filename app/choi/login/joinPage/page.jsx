...
const JoinPage = () => {
  ...
  async function handleIdCheck() {
    ...
    if (!idPattern.test(uvo.user_id)) {
      ...
      try {
        const response = await axios.post("/user/id_check", {
          user_id: uvo.user_id,
        });
        if (response.data === 1) {
          alert("중복아이디 있음");
  ...
  const handleEmailCheck = async () => {
    ...
    try {
      const response = await axios.post("/user/email_check", {
        email: fullEmail,
      });
  ...
  const handleSubmit = async () => {
    ...
    if (!name || !user_id || !pwd || !email) {
      ...
      alert(
        "비밀번호는 영문, 숫자, 특수문자(~!@#$%^&*)를 포함하여 8~15자리여야 합니다."
      );
    } else {
      ...
      <Background>
        <JoinBox>
          <Wrapper_All>
            ...
            />
            <Error>
              {uvo.pwd !== uvo.repwd && uvo.repwd && <>비밀번호 불일치!</>}
            ...
            <IdContainer>
              <Email_Box>
                ...
                <IdCheckButton type="button" onClick={handleEmailCheck}>
                  중복확인
                </IdCheckButton>
              </Email_Box>
            </IdContainer>
            <NoticeAllBox>
              ...
              <NoticeBox>
                <CheckBox_Basic
                  type="checkbox"
                  checked={requiredChecks.age}
                  onChange={() => handleRequiredCheckChange("age")}
                />
                <Notice_Title_Basic>
                  [필수] 만 14세 이상입니다.
                </Notice_Title_Basic>
              ...
              <NoticeBox>
                <CheckBox_Basic
                  type="checkbox"
                  checked={requiredChecks.terms}
                  onChange={() => handleRequiredCheckChange("terms")}
                />
                <Notice_Title_Basic>
                  [필수] 서비스 이용약관 동의
                </Notice_Title_Basic>
              ...
              <NoticeBox>
                <CheckBox_Basic
                  type="checkbox"
                  checked={requiredChecks.privacy}
                  onChange={() => handleRequiredCheckChange("privacy")}
                />
                <Notice_Title_Basic>
                  [필수] 개인정보 수집 및 이용 동의
                </Notice_Title_Basic>
              ...
              <NoticeBox>
                <CheckBox_Choice
                  type="checkbox"
                  checked={optionalChecks.privacyOptional}
                  onChange={() => handleOptionalCheckChange("privacyOptional")}
                />
                <Notice_Title_Choice>
                  [선택] 개인정보 수집 및 이용 동의
                </Notice_Title_Choice>
              ...
              <NoticeBox>
                <CheckBox_Choice
                  type="checkbox"
                  checked={optionalChecks.marketing}
                  onChange={() => handleOptionalCheckChange("marketing")}
                />
                <Notice_Title_Choice>
                  [선택] 마케팅 정보 수신 동의{" "}
                </Notice_Title_Choice>
